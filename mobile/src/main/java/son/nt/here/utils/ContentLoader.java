    ...
    private final AsyncTask<ContentManager, Void, T> task = new AsyncTask<ContentManager, Void, T>() {
        ...
        protected T doInBackground(ContentManager... params) {
            ...
            try {
                ...
                if (value != null) {
                    Logger.debug(TAG, "from cache: " + value);
                    return value;
                }
                ...
                return client.execute(request, new ResponseHandler<T>() {
                    ...
                    public T handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
                        ...
                        Logger.debug(TAG, "status: " + stat + " " + getRequest().getURI().toASCIIString());
                        ...
                        try {
                            ...
                            if (useCache) {
                                ...
                                synchronized (manager) {
                                    ...
                                    Logger.debug(TAG, "put cache " + fname + " " + getRequest().getURI().toASCIIString());
                                    manager.putCache(fname, in);
                                }
                                in = new ByteArrayInputStream(data.toByteArray());
                            }
                            Logger.debug(TAG, "from network: " + getRequest().getURI().toASCIIString());
        ...
        try {
            synchronized (manager) {
                ...
                if (isValidCache(f) == false) {
                    Logger.debug(TAG, "invalid cache " + key);
